...
class PGMExplainer(Explainer):
    ...
    def __init__(self,
        ...
        self.explainer = torchExplainerRunner(
            model=model,
            algorithm=PGMExplainerAlgorithm(
                feature_index=None,
                perturbation_mode=perturbation_mode,
                perturbations_is_positive_only=perturbations_is_positive_only,
                is_perturbation_scaled=is_perturbation_scaled,
    ...
    def run(self, mode, kwargs, finalize=True):
        ...
        if self.gen_dataset.is_multi():
            ...
            self.y = self.gen_dataset.data.y

        self.pbar.reset(total=1)
        if self.gen_dataset.is_multi():
            self.raw_explanation = self.explainer(self.x, self.edge_index, target=self.y)
        else:
            self.raw_explanation = self.explainer(self.x, self.edge_index, target=self.y, index=self.node_idx)
        self.pbar.close()
    ...
    def _finalize(self):
        ...
        for i in range(num_nodes):
            feat_imp = map(float, set(node_mask[i]))
            imp = next((x for x in feat_imp if x >= eps), False)
            if imp:
...
class PGMExplainerAlgorithm(ExplainerAlgorithm):
    ...
    def _explain_node(
        ...
        **kwargs,
        ...
        if self.max_subgraph_size is None:
            ...
            # MY CODE (The problem is the name of the first two keys: 'A', 'B'. Solution to rename to 0, 1)
            key1, key2 = list(index_subgraph_to_original.keys())[:2]
            index_subgraph_to_original[0] = index_subgraph_to_original.pop(key1)
            index_subgraph_to_original[1] = index_subgraph_to_original.pop(key2)
            # # # # #
